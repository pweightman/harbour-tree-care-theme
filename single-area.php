...
while ( have_posts() ) :
	...
	harbour_page_hero(
		array(
			'crumbs'  => array(
				...
				array(
					'label' => __( 'Areas', 'harbour-tree-care' ),
			...
			'buttons' => array(
				array(
					'label' => __( 'Book a free site visit', 'harbour-tree-care' ),
			...
			<div class="two-col">
				...
				<aside class="sidebar">
					...
					if ( $services ) :
						?>
						<div class="side-card">
							<h3><?php /* translators: %s: area name. */ printf( esc_html__( 'Services in %s', 'harbour-tree-care' ), esc_html( get_the_title() ) ); ?></h3>
							<ul>
	...
	if ( function_exists( 'harbour_reviews_render' ) ) {
		...
		if ( $reviews_html ) {
			echo '<section class="section bg-cream"><div class="wrap"><div class="sec-head reveal"><div class="measure"><p class="eyebrow">' . esc_html__( 'What people say', 'harbour-tree-care' ) . '</p><h2>' . esc_html( /* translators: %s: area name. */ sprintf( __( 'Reviews from around %s.', 'harbour-tree-care' ), get_the_title() ) ) . '</h2></div></div>';
	...
	harbour_cta_band(
		array(
			'heading' => /* translators: %s: area name. */ sprintf( __( 'Got a tree in %s that needs looking at?', 'harbour-tree-care' ), get_the_title() ),
